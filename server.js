...
router.get('/getCaseAttachments', function(req, res) {
    ...
    pg.connect(process.env.DATABASE_URL, function (err, conn, done) {
        if (err) console.log(err);
        conn.query(
            'SELECT id, herokucaseid, name, contenttype, body  from caseattachment where herokucaseid= '+case_id+'',
            function(err,result){
                done();
                if(err){
                    return res.status(400).json({error: err.message});
...
router.get('/getProducts', function(req, res) {
    ...
    pg.connect(process.env.DATABASE_URL, function (err, conn, done) {
        if (err) console.log(err);
        conn.query(
            'SELECT sfid, Name, Product_Type__c, Type__c, Product_Description__c, Product_Italy_Description__c, Product_Germany_Description__c, Serial_Batch_code__c, Serial_Batch_code_format__c, Serial_Batch_code_contains__c, BrandingColor__c FROM salesforce.FMA_Product__c WHERE Type__c=\''+type+'\'',
            function(err,result){
                done();
                if(err){
                   return res.status(400).json({error: err.message});
